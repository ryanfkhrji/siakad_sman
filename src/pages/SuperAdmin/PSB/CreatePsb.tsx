...
import { Upload, AlertCircle, Loader2, AlertCircleIcon, CircleXIcon, FilePlus } from "lucide-react";
...
const CreatePsb = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await psbService.create(formData);
  ...
  return (
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[300px]"}`}>
        <PageTitle title="Tambah Pendaftaran Siswa Baru" />

        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold mb-6">Tambah Pendaftaran Siswa Baru</h1>
