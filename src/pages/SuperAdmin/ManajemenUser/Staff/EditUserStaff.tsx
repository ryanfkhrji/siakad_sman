...
const EditUserStaff = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (res.data.status === "success") {
          ...
          Swal.fire("Error", res.data.message || "Gagal mengambil data staff", "error");
        }
      } catch (error) {
  ...
  const handleUpdate = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!pegawai?.nip) {
      setErrors((prev) => ({ ...prev, nisn: "Data staff tidak valid" }));
      return;
    }
    ...
    try {
      ...
      if (res.data?.status === "success") {
        ...
        navigate("/superadmin/manajemen-user/staff");
      } else {
  ...
  return (
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[300px]"}`}>
        <PageTitle title="Edit User Staff" />

        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold mb-6">Edit User Staff</h1>
          ...
          {loading ? (
              ...
              <div className="bg-white rounded shadow p-5">
                ...
                <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-1 md:col-span-2 flex gap-2 mt-4">
                    ...
                    <Link to="/superadmin/manajemen-user/staff">
            ...
            <p className="text-center text-gray-600 mt-6">Data staff tidak ditemukan.</p>
          )}
        </div>
