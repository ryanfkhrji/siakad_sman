...
const UserKepsek = () => {
  ...
  const handleDelete = async (id: number) => {
    ...
    const result = await Swal.fire({
      ...
      text: "Data kepala sekolah yang dihapus tidak dapat dikembalikan.",
      icon: "warning",
    ...
    try {
      ...
      if (res.data.status === "success") {
        ...
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
          text: "Data kepala sekolah berhasil dihapus.",
        ...
        Swal.fire({
          ...
          text: res.data.message || "Terjadi kesalahan saat menghapus kepala sekolah.",
        });
      }
    } catch (err: any) {
      ...
      if (err.response?.data?.status === "error") {
        Swal.fire({
          icon: "error",
          title: "Gagal menghapus!",
          text: err.response.data.message || "Kepala sekolah tidak ditemukan.",
        });
      } else {
        Swal.fire({
          icon: "error",
      ...
      console.error("Gagal menghapus kepala sekolah:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SidebarProvider>
      ...
      `}
      >
        <PageTitle title="Data User Kepala Sekolah" />
        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold mb-6">Data User Kepala Sekolah</h1>
