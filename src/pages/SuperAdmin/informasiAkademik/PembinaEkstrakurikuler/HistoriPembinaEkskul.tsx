...
import type { DetailPembinaEkskul } from "@/types/pembinaEkskul";

// Data dasar pembina dikirim via router state dari index
interface PembinaNavState {
  nama_pembina: string;
  nip: string;
  nuptk: string;
}

const HistoriPembinaEkskul = () => {
  const { id: pembinaId } = useParams<{ id: string }>();
  const navigate  = useNavigate();
  const location  = useLocation();
  const navState  = location.state as PembinaNavState | null;

  const [isCollapsed,   setIsCollapsed]   = useState(false);
  const [loading,       setLoading]       = useState(true);
  const [deleting,      setDeleting]      = useState(false);
  const [histori,       setHistori]       = useState<DetailPembinaEkskul | null>(null);
  const [expandedPeriode, setExpandedPeriode] = useState<Set<number>>(new Set());

  // ── Fetch histori ──────────────────────────────────────────────
  useEffect(() => {
    if (pembinaId) fetchHistori();
  }, [pembinaId]);

  const fetchHistori = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/spa/pembina/ekstrakurikuler/${pembinaId}`);
      if (res.data.status === "success") {
        const data: DetailPembinaEkskul = res.data.data;
        setHistori(data);
      ...
      const msg =
        ...
        "Pembina belum pernah ditugaskan di ekskul.";
  ...
  const handleDelete = async (ekskulId: number, tahunAkademikId: number) => {
    const confirm = await Swal.fire({
      ...
      text: "Data pembina pada periode ini akan dihapus.",
      icon: "warning",
    ...
    try {
      ...
      const pivotId: number | null =
        matchPeriode?.anggota?.pembina?.pembina_pivot_id ?? null;

      if (!pivotId) {
        Swal.fire({ icon: "error", title: "Gagal", text: "Pivot ID pembina tidak ditemukan." });
        return;
      }

      const res = await api.delete(`/spa/pembina/ekstrakurikuler/${pivotId}`);
      if (res.data.status === "success") {
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
        ...
        await fetchHistori(); // refresh data
      }
    } catch (err: any) {
      Swal.fire({
        icon: "error",
  ...
  // Nama pembina: dari response API (lebih akurat) atau fallback dari nav state
  const namaPembina = histori?.nama_pembina ?? navState?.nama_pembina ?? "...";
  ...
  return (
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[300px]"}`}>
        <PageTitle title={`Histori Membina — ${namaPembina}`} />
        ...
        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4 mb-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate("/superadmin/informasi-akademik/pembina-ekskul")}
            >
              <ArrowLeftIcon size={16} /> Kembali
            </Button>
            <h1 className="text-3xl font-bold">Histori Membina</h1>
          </div>

          {/* Info pembina */}
          {!loading && (
            <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-800 flex flex-wrap gap-6">
              <span><span className="font-medium">Nama:</span> {namaPembina}</span>
              <span><span className="font-medium">NIP:</span> {nip}</span>
              <span><span className="font-medium">NUPTK:</span> {nuptk}</span>
            </div>
          )}
          ...
          {loading ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-white text-sm opacity-80">
                        {periode.ekstrakurikuler.length} ekskul
                      </span>
                    </div>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow className="bg-gray-50">
                              ...
                              {/* Kolom aksi hanya tampil untuk periode aktif */}
                              {!isArsip && (
                            ...
                            {periode.ekstrakurikuler.map((ekskul) => (
                              ...
                              >
                                ...
                                <TableCell className="text-right text-sm">
                                  {formatRupiah(Number(ekskul.anggaran))}
                                </TableCell>
                                ...
                                {!isArsip && (
                                  <TableCell className="text-center">
                                    <div className="flex justify-center gap-1">
                                      <Link
                                        to={`/superadmin/informasi-akademik/pembina-ekskul/edit/${ekskul.ekskul_id}`}
                                        state={{
                                          pembinaData: {
                                            ekskul_id: ekskul.ekskul_id,
                                            nama_ekskul: ekskul.nama_ekskul,
                                            pembina_id: histori.pembina_id,
                                            nama_pembina: histori.nama_pembina,
                                      ...
                                      >
                                        <Button size="sm" title="Edit pembina">
                                          <PenBoxIcon size={14} />
                                        </Button>
                                      </Link>
            ...
            !loading && (
              <div className="text-center text-gray-500 py-16">
                Belum ada histori membina untuk pegawai ini.
              </div>
            )
          )}
        </div>
...
export default HistoriPembinaEkskul;