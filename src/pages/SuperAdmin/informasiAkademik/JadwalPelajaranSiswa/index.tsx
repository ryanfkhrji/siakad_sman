...
import type { JadwalPelajaranSiswa } from "@/types";
import Swal from "sweetalert2";

const DataJadwalPelajaranSiswa = () => {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [dataSiswa, setDataSiswa] = useState<JadwalPelajaranSiswa[]>([]);
  const [filteredSiswa, setFilteredSiswa] = useState<JadwalPelajaranSiswa[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (res.data.status === "success") {
          const siswaOnly = res.data.data.filter((p: JadwalPelajaranSiswa) => p.role === "siswa");
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="mb-6 flex flex-col md:flex-row justify-between items-center gap-4 w-full">
                <Link to="/superadmin/informasi-akademik/jadwal-pelajaran-siswa/create" className="w-full md:w-auto">
                  <Button className="bg-primary w-full md:w-auto">
                    <PlusIcon size={18} />
                    Tambah Jadwal Pelajaran Siswa
                  </Button>
                </Link>
