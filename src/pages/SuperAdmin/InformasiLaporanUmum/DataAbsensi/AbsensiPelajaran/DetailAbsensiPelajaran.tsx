...
interface AbsensiDetailPelajaran {
  id: number;
  kelas: string;
  hari: string;
  jam: string;
...
interface DetailAbsensiDataPelajaran {
  guru_id: number;
  nama_guru: string;
  mengajar: string;
  total_hadir: number;
...
const DetailAbsensiPelajaran = () => {
  ...
  const [detailData, setDetailData] = useState<DetailAbsensiDataPelajaran | null>(null);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredData, setFilteredData] = useState<AbsensiDetailPelajaran[]>([]);
  ...
  const fetchDetailData = async () => {
    try {
      setLoading(true);
      const response = await absensiPelajaranService.getDetail(Number(id));
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() === "") {
      ...
      setFilteredData(detailData.absensi.filter((item) => item.kelas.toLowerCase().includes(lower) || item.hari.toLowerCase().includes(lower) || item.jam.toLowerCase().includes(lower) || item.status.toLowerCase().includes(lower)));
    }
    setCurrentPage(1);
  ...
  const handleExportExcel = async (selected: boolean = false) => {
    try {
      ...
      link.href = url;
      link.download = `absensi-pelajaran-${detailData?.nama_guru.replace(/\s+/g, "-")}.xlsx`;
      link.click();
  ...
  return (
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[300px]"}`}>
        <PageTitle title={`Detail Absensi Pelajaran - ${detailData?.nama_guru || "Loading..."}`} />
            ...
            <>
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-gray-900">{detailData.total_hadir + detailData.total_tidak_hadir} Kali Mengajar</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="mb-6 flex flex-col gap-4">
                ...
                <div className="relative w-full md:w-1/3">
                  ...
                  <Input type="text" placeholder="Cari kelas/hari/jam/status..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-8" />
                </div>
              </div>
