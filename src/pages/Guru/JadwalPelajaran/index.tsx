...
import { SearchIcon, Loader2Icon, ClockIcon, BookOpenIcon, CalendarIcon, DoorOpenIcon, LinkIcon, InfoIcon } from "lucide-react";
...
interface DataJadwalGuru {
  nama: string;
  jadwal: JadwalPelajaran[];
}

const JadwalPelajaranGuru = () => {
  const navigate = useNavigate();
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [dataJadwal, setDataJadwal] = useState<DataJadwalGuru | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await api.get("/pegawai/jadwal-pelajaran/all/diri");

        if (res.data.status === "success") {
          setDataJadwal(res.data.data);
          setFilteredJadwal(res.data.data.jadwal);
        }
      } catch (error: any) {
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() === "") {
      setFilteredJadwal(dataJadwal.jadwal);
    } else {
      const lowerSearch = searchTerm.toLowerCase();
      const filtered = dataJadwal.jadwal.filter((item) => item.mata_pelajaran.toLowerCase().includes(lowerSearch) || item.ruangan.toLowerCase().includes(lowerSearch) || item.hari.toLowerCase().includes(lowerSearch));
              ...
              <Card className="mb-6 bg-white border-l-4 border-l-primary">
                <CardContent>
                  <div className="flex justify-between items-center flex-wrap gap-4 text-sm">
                    <div>
                      <p className="text-gray-600 font-medium">Nama Guru</p>
                      <p className="text-lg font-bold text-primary">{dataJadwal.nama}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                    ...
                    filteredJadwal.map((jadwal) => (
                      <Card className="w-full hover:shadow-lg transition-shadow" key={jadwal.id}>
                        ...
                        <CardFooter className="flex-col gap-2">
                          <Button className="w-full" onClick={() => navigate(`/guru/jadwal-pelajaran/detail/${jadwal.id}`)}>
                            <InfoIcon size={18} />
                            Lihat Detail
                          </Button>
                        </CardFooter>
