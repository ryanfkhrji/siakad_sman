...
const EditPasswordSiswa = () => {
  ...
  const validatePasswords = () => {
    ...
    if (passwordBaru.length < 5) err.password_baru = "Password minimal 5 karakter";

    // Validasi regex untuk password baru
    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).+$/;
    if (passwordBaru && !passwordRegex.test(passwordBaru)) {
      err.password_baru = "Password harus mengandung huruf besar, huruf kecil, angka, dan simbol";
    }
    ...
    if (passwordBaru !== confirmPassword) {
      err.konfirmasi_password = "Konfirmasi password tidak cocok";
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (user.role !== "siswa") {
          Swal.fire("Error", "User ini bukan siswa", "error");
          return;
        }
        ...
        Swal.fire("Error", "Terjadi kesalahan mengambil data siswa", "error");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUpdate = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await api.put("/siswa/ubah-password/diri", payload);
  ...
  return (
    <SidebarProvider>
      ...
      <main className={`w-full min-h-screen bg-background transition-all duration-300 ${isCollapsed ? "md:ml-16" : "md:ml-[280px]"}`}>
        <PageTitle title="Edit Password Siswa" />

        <div className="mx-auto p-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold mb-6">Edit Password Siswa</h1>
          ...
          {loading ? (
            ...
            <p className="text-center text-gray-600 mt-6">Data siswa tidak ditemukan.</p>
          ) : (
            <div className="bg-white rounded shadow p-5">
              ...
              <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" value={siswa.nisn} readOnly disabled className="border p-2 w-full mt-2 rounded bg-gray-100 cursor-not-allowed" />
                </div>

                <div>
                  <label className="block font-semibold">NIS</label>
                  <input type="text" value={siswa.nis} readOnly disabled className="border p-2 w-full mt-2 rounded bg-gray-100 cursor-not-allowed" />
                </div>

                <div>
                  <label className="block font-semibold">Nama Lengkap</label>
                  <input type="text" value={siswa.nama} readOnly disabled className="border p-2 w-full mt-2 rounded bg-gray-100 cursor-not-allowed" />
                </div>

                <div>
                  <label className="block font-semibold">Email</label>
                  <input type="text" value={siswa.email} readOnly disabled className="border p-2 w-full mt-2 rounded bg-gray-100 cursor-not-allowed" />
                </div>

                <div>
                  <label className="block font-semibold">Status</label>
                  <input type="text" value={siswa.status ?? "Aktif"} readOnly disabled className="border p-2 w-full mt-2 rounded bg-gray-100 cursor-not-allowed" />
                </div>

                <div>
                  <label className="block font-semibold">Role</label>
                  <input type="text" value={siswa.role ?? "siswa"} readOnly disabled className="border p-2 w-full mt-2 rounded bg-gray-100 cursor-not-allowed" />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Min 5 karakter (huruf besar, kecil, angka, simbol)"
                ...
                {/* Tombol */}
                <div className="col-span-1 md:grid-cols-2 flex gap-2 mt-4">
...
export default EditPasswordSiswa;
